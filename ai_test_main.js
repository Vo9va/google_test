...
async function handleGitHubFlow(issueKey = 'TT-16') {
  const branchName = `auto-test/${issueKey}`;
  const repoOwner = 'Vo9va'; // Твій юзер з логів
  const repoName = 'google_test';

  try {
    // 1. Пуш у нову гілку
    const remote = `https://${process.env.GH_TOKEN}@github.com/${repoOwner}/${repoName}.git`;
    await git.checkoutLocalBranch(branchName);
    await git.add('./*');
    await git.commit(`feat: auto-test for ${issueKey}`);
    await git.push(remote, branchName);

    // 2. Створення Pull Request через API
    const prResponse = await axios.post(
        `https://api.github.com/repos/${repoOwner}/${repoName}/pulls`,
        {
          title: `[AI] Auto-test for ${issueKey}`,
          head: branchName,
          base: 'main',
          body: `Цей тест був автоматично згенерований для задачі ${issueKey}`
        },
        {
          headers: { 'Authorization': `token ${process.env.GH_TOKEN}` }
        }
    );
    console.log(`🚀 PR створено: ${prResponse.data.html_url}`);
  } catch (err) {
    console.error('Помилка автоматизації GitHub:', err.response?.data || err.message);
  }
}
